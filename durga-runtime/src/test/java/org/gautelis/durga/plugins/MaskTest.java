...
public class MaskTest {
    ...
    public void shouldMaskDefaultChar() throws Exception {
        ...
        String result = Mask.mask("{\"ssn\":\"123-45-6789\"}", "ssn", '*', 0);
    ...
    public void shouldPreserveBoundaryChars() throws Exception {
        ...
        String result = Mask.mask("{\"email\":\"alice@example.com\"}", "email", '*', 3);
    ...
    public void shouldPreserveFourChars() throws Exception {
        ...
        String result = Mask.mask("{\"card\":\"4111111111111111\"}", "card", '#', 4);
    ...
    public void shouldPassthroughOnNullFields() {
        ...
        String input = "{\"a\":1}";
        assertEquals(input, Mask.mask(input, "", '*', 0));
        assertEquals(input, Mask.mask(input, null, '*', 0));
    }

    @Test
    public void shouldPassthroughOnShortText() {
        System.out.println("TC: fully masks text shorter than preserve window");
        String result = Mask.mask("{\"x\":\"ab\"}", "x", '*', 5);
    ...
    public void shouldHandleEmptyString() {
        ...
        String result = Mask.mask("{\"x\":\"\"}", "x", '*', 0);
        assertEquals("{\"x\":\"\"}", result);
    }

    @Test
    public void shouldExecuteViaPluginInterface() throws Exception {
        System.out.println("TC: execute parses fields, mask and preserve config");
        Plugin plugin = new Mask();
        byte[] result = plugin.execute(
                Plugin.toBytes("{\"email\":\"alice@example.com\"}"),
                "fields=email;mask=#;preserve=2");
        assertEquals("{\"email\":\"al#############om\"}", Plugin.toString(result));
    }

    @Test(expected = IllegalArgumentException.class)
    public void shouldThrowOnInvalidJson() {
        System.out.println("TC: throws on malformed input JSON");
        Mask.mask("not json", "a", '*', 0);
    }
}
