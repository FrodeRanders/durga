\section{System Overview}
\label{sec:SystemOverview}

Durga has three parts:
\begin{itemize}
\item a BPMN parsing and generation pipeline,
\item a generated Kafka-oriented process runtime,
\item a monitoring and reporting path built around canonical lifecycle events.
\end{itemize}

\subsection{Repository-level components}

\begin{description}
\item[Scaffolder] Parses a BPMN model and renders a generated project \\ (entry point:
\texttt{org.gautelis.durga.tools.BpmnScaffolder}).
\item[Generation support] Model extraction and template coordination: \texttt{BpmnModelSupport}, \\
\texttt{TaskRoutingGenerator}, \texttt{EventTemplateGenerator},
\texttt{SubProcessTemplateGenerator}, \texttt{GeneratedProjectSupport}.
\item[Templates] StringTemplate groups under \texttt{durga-tools/src/main/resources/templates-java/}
define the generated Java classes, scripts, YAML, Maven build files and shared runtime
helpers. A parallel \texttt{templates-rust/} directory holds the equivalent templates for
the in-progress Rust code-generation target (see Section~\ref{sec:RustTarget}).
\item[Monitoring runtime] A Kafka Streams topology consumes canonical lifecycle events and
materializes state, counts, latency summaries and stuck-instance views.
\end{description}

\subsection{Shape of generated runtime}

Each generated project contains:
\begin{itemize}
\item worker and routing services,
\item a starter,
\item a process orchestrator,
\item helpers for user/manual task completion and failure testing,
\item generated Kafka topic configuration,
\item sample payloads and runnable scenario scripts.
\end{itemize}

Topics and handlers are deliberately visible so that BPMN concepts are projected concretely
onto Kafka channels and lifecycle events.

\begin{figure}[!ht]
    \centering
    \begin{tikzpicture}[node distance=16mm,>=latex]
        \tikzstyle{box}=[draw=iptoMuted, rounded corners=2pt, line width=0.8pt, align=center, minimum width=34mm, minimum height=11mm, fill=white]
        \tikzstyle{modelbox}=[box, fill=iptoAccent!12]
        \tikzstyle{transformbox}=[box, fill=red!12]
                
        \node[modelbox] (bpmn) {BPMN model\\input};
        \node[transformbox, right=10mm of bpmn] (scaffolder) {Durga scaffolder\\Camunda + ST4};
        \node[box, right=10mm of scaffolder] (generated) {Generated process\\project (fat jar)};
        \node[box, right=10mm of generated] (topics) {Execution topics\\and handlers};

        \node[box, below=10mm of scaffolder] (events) {Canonical\\process-events-\{processId\}};
        \node[modelbox, below=10mm of generated] (models) {Process-models\\(BPMN registry)};

        \node[box, below=10mm of models] (monitoring) {Kafka Streams\\monitoring};

        \draw[->, thick] (bpmn) -- (scaffolder);
        \draw[->, thick] (scaffolder) -- (generated);
        \draw[->, thick] (generated) -- (topics);
        \draw[->, thick] (generated) -- (events);
        \draw[->, thick] (generated) -- (models) node[midway, right, font=\footnotesize] {auto-registers on boot};
        \draw[->, thick] (events) -- (monitoring);
        \draw[->, thick] (models) -- (monitoring);
    \end{tikzpicture}
    \caption{High-level flow: BPMN model $\to$ generated runtime $\to$ self-registration $\to$ monitoring}
    \label{fig:DurgaOverview}
\end{figure}

\subsection{Process self-registration}

Each generated process publishes its BPMN 2.0 model to the \texttt{process-models}
Kafka topic on startup via a \texttt{ModelRegistration} bean. The monitoring
server discovers processes from this registry — no pre-configured process ID
list is needed. The monitoring topology subscribes to all \texttt{process-events-*}
topics via a regex pattern, making every process visible in a single monitoring
instance.

\subsection{Canonical event model}

The canonical lifecycle-event stream --- by default a per-process topic
\texttt{process-events-\textit{processId}}, which the monitor consumes as the
\texttt{process-events-*} family --- is the observability backbone for
latest state per instance, counts by current state, latency summaries, stuck-instance
detection, dashboards and query endpoints. Execution topics may be useful for routing
visibility, but the monitoring model depends on explicit projections rather than raw topic
inspection.
