...
When scaffolded with \texttt{--validation}, the output additionally contains a
validation-mode shadow worker per plugin task
(\texttt{\textit{Name}ValidationWorker.java}) and the
\texttt{ValidationCandidateOutput} runtime contract, with the dedicated validation
consumer group and the \texttt{validation-candidate-outputs} channel merged into
\texttt{application.yml} and the validation topics added to the topic-creation
script (Section~\ref{sec:ValidationMode}).
