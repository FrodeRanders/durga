...
model: workers can crash and restart independently without losing messages.
...
java -jar target/durga-1.0-SNAPSHOT.jar \
  src/test/resources/bpmn/invoice_receipt.bpmn --separate-workers
\end{verbatim}
...
\item[Message durability] Messages are not lost on worker crash. Kafka
      retains them until consumed and committed.
\item[At-most-once vs at-least-once] Standalone workers commit after
      ...
      idempotent output being the operator's responsibility.
...
\item Worker state is external (Kafka). In-memory state such as running
      aggregations or cancellation registrations is lost on crash.
