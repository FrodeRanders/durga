...
\item[shouldParseAlarmConfigsFromBpmnModel] Parses the E2E fixture with
\texttt{BpmnAlarmConfigParser} and asserts that alarm configurations are
present at all three levels: an activity-level \texttt{HARD\_ERROR} alarm
on \texttt{validate\_high\_value}, a process-level inherited
\texttt{COUNTED} alarm (threshold~5) applied to every activity, and a
process-level aggregate \texttt{SLIDING\_WINDOW} alarm (threshold~3,
60~s window).
...
\subsection{ValidationTopologyIntegrationTest}
\label{it:validationTopology}

\textbf{Purpose:} Verifies the validation (shadow-run) topology end to end
against a live Kafka broker: candidate-task outputs published to the
validation-candidate topic are joined against production outputs and the
comparison outcome is materialised into the validation results store.

\textbf{Scenarios (6):}

\begin{description}

\item[shouldMarkEqualWhenCandidateMatchesPriorRegardlessOfArrivalOrder] A
candidate output that matches the production output resolves to
\texttt{EQUAL}, even when the candidate arrives before the prior output
(the join is order-independent).

\item[shouldMarkDiffWithLocatedDifference] A differing candidate output is
recorded as \texttt{DIFF} together with the located difference.

\item[shouldMarkPriorMissingWhenNoProductionOutput] A candidate with no
corresponding production output is recorded as \texttt{PRIOR\_MISSING}.

\item[shouldMarkCandidateError] A candidate that reports an execution error
is recorded as \texttt{CANDIDATE\_ERROR} even when a prior output exists.

\item[shouldIgnoreConfiguredPaths] Differences under a configured ignore
path (e.g. volatile fields) do not count as a divergence.

\item[shouldSummarizePerTask] The per-task validation summary correctly
counts \texttt{EQUAL} and \texttt{DIFF} outcomes.

\end{description}

\textbf{Why it matters:} Validation mode lets a candidate task be shadow-run
against production without affecting the live pipeline; this suite proves the
comparison outcomes are computed and materialised correctly over real Kafka.

% ===========================================================================

\subsection{SlaMonitoringIntegrationTest}
\label{it:slaMonitoring}

\textbf{Purpose:} End-to-end verification of the performance-SLA alarm path. Runs
the fault-detection topology (which measures SLAs from lifecycle events and emits
alarms to \texttt{fault-alarms}) together with the alarm-state topology (which
folds those alarms into the queryable read model consumed by \texttt{/api/metrics}),
both against a live Kafka broker.

\textbf{Scenarios (2):}

\begin{description}

\item[shouldFireSlaLatencyAlarmWhenTaskExceedsLimit] Publishes
\texttt{ACTIVITY\_ENTERED} then \texttt{ACTIVITY\_COMPLETED} four minutes apart for a
task with a 1\,s \texttt{SLA\_LATENCY} ceiling. Asserts the alarm-state store
materialises an \texttt{SLA\_LATENCY} alarm whose observed latency
(\texttt{lastCount}) is 240\,000\,ms and limit (\texttt{threshold}) is 1\,000\,ms ---
the exact values surfaced as \texttt{durga\_sla\_last\_observed} /
\texttt{durga\_sla\_limit}.

\item[shouldFireSlaThroughputAlarmWhenRateStarves] Declares a process-wide
\texttt{SLA\_THROUGHPUT} floor of two completions per two-second window, feeds a
single completion, and asserts the wall-clock punctuator reports a breach
(\texttt{lastCount} below \texttt{threshold}) once a full window has elapsed with
insufficient throughput. Uses a 1\,s scan interval override for prompt scoring.

\end{description}

\textbf{Why it matters:} Proves that SLAs declared in the process model turn into
real, queryable alarms driven by observed wall-clock latency and throughput ---
not merely a static counter.
...
\textbf{KafkaIntegrationTestBase} (\filepath{durga-runtime/src/test/java/}
\filepath{org/gautelis/durga/KafkaIntegrationTestBase.java}) uses the Testcontainers
