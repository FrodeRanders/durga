...
plus the fault detection unit test suite for completeness.
All tests extend \texttt{KafkaIntegrationTestBase}, which uses Testcontainers
to spin up a real Kafka broker (Confluent CP 7.8.0 in KRaft mode) per test
class. Each test class creates its own topics with randomised suffixes to
avoid collisions and disposes of all resources in \texttt{@After} /
\texttt{@AfterClass} hooks.
...
\textbf{Fixture:} \filepath{src/test/resources/bpmn/e2e\_pipeline.bpmn} ---
...
\textbf{Class:} \texttt{FaultDetectionTest} (13 scenarios, no Docker required).
...
All six suites share a common base class for Docker-backed tests:
...
\texttt{mvn test -Dtest='*IntegrationTest' -Ddurga.integration.requireDocker=true}
\end{itemize}
