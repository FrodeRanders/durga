...
cancellation. Each process uses a dedicated topic \texttt{process-events-\{processId\}}
so that process definitions are isolated at the transport level.
...
The \texttt{/api/diagram?processId=\textit{id}} endpoint serves the cached model, with
a fallback to a local BPMN directory for development convenience. Model updates
overwrite previous versions (latest-wins semantics).
...
The monitoring topology can run in two modes:

\begin{description}
\item[Single-process] Subscribes to \texttt{process-events-\{processId\}}.
  Suitable for dedicated monitoring per process definition.
\item[Multi-process] Subscribes to \texttt{process-events-.*} via a regex pattern.
  All processes share one monitoring instance. State stores use unsuffixed names.
  The SPA shows an inventory of all known processes with drill-down to per-process detail.
\end{description}
...
The main entry point is \texttt{org.gautelis.durga.monitoring.MonitoringContainer}.
Pass \texttt{all} as the process ID argument for multi-process mode.
...
Supported by both the embedded JDK HTTP server and the Quarkus REST container.
All paths accept an \texttt{/api/} prefix for SPA proxy compatibility.
