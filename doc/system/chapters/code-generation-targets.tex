...
\texttt{ProcessEvent}, dot-path field helpers) and the first faithful plugin ports
(\texttt{json-transform}, \texttt{type-coercer}) with unit tests. Remaining work:
the rest of the plugin ports, the Rust worker / gateway / orchestrator and
project (\texttt{Cargo.toml}) templates, and the \texttt{-{}-target rust} switch in
the CLI. The Rust workers will use a mature Kafka client
(\texttt{rdkafka}) and an async runtime (\texttt{tokio}) for the same
consume-transform-emit loop the plain-Kafka Java worker performs.
