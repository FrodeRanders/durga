...
The low-level Charlotte Kafka profile supports one fixed consume route and a
bounded, ordered allow-list of produce topic/partition routes. Producer sequence
numbers and transaction partition state are maintained per route. A version-2
profile is serialized as one SHA-256-authenticated object and passed through a
kernel-enforced read-only launch capability. Generated descriptors select a
maximum of 64 produce routes; operators may lower that admission ceiling, while
Charlotte rejects either a declared limit or an actual route count above its
hard maximum. The missing
higher-level contract is the generic transactional-step runner and its bounded
procedure request/reply ABI. Generated deployments therefore say
\texttt{deployable: false} until that runner, capability injection, and the
activity handlers exist; the target does not silently weaken Durga's delivery
semantics.
...
profile containing broker addresses, TLS material, and future SASL or mTLS
credentials. The runner receives an attenuated connection capability to that
connector, and the activity procedure receives only its bounded invocation.
