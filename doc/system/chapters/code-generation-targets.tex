...
partition state are maintained per route. A version-4 profile is serialized as
...
establishes provenance. It also fixes the connector's bounded registration
name, so generated steps use independently named connector instances rather
than replacing one global \texttt{kafka} publication. Generated descriptors select maxima of 32 broker
