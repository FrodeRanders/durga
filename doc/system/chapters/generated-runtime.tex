...
Each generated project contains Java classes under the configured base package
(\texttt{--package}, default \texttt{org.example.generated}). In the examples
below this package is written as \texttt{<pkg>}.
Operational classes (workers, gateways, orchestrator) are generated under:
\filepath{src/main/java/<pkg>/}.
Probes and exercisers (starter, scenario runner, publishers, watchers) are
generated under:
\filepath{src/main/java/<pkg>/probes/}.
Shared runtime contract classes (\texttt{ProcessEvent}, \texttt{DataHandle},
plugin runtime, etc.) are generated under
\filepath{src/main/java/org/gautelis/durga/}.
...
output, per-process lifecycle, and dead-letter topics. The difference is in how the
...
canonical \texttt{process-events-\{processId\}} emission. They also emit a Vannak-compatible
\texttt{DataIndividualMetadataEvent} to the \\ \texttt{vannak-metadata-events-\{processId\}} topic
