...
This document is written to help you argue for Durga in a room where the
default answer is Airflow. It assumes the audience knows what a data
pipeline is and has opinions about how to build one.
...
## The five questions a decision-maker will ask
...
validation step, implement the `Plugin` interface. The 15 built-in
...
**Write a custom script in the BPMN model.** `ScriptTask` elements
support inline Groovy/JavaScript. The script lives in the BPMN file —
it survives regeneration. Durga currently generates a stub with `// TODO`
markers; implementing script execution is straightforward.

**Modify the generated worker.** The generated code is clean Java. You
can add custom logic. On regeneration, existing files under `src/main/java`
are skipped — your modifications survive. For gateways, the condition
expressions in the BPMN model are evaluated at runtime by a recursive
descent parser supporting `==`, `!=`, `>`, `<`, `>=`, `<=`, `&&`, `||`,
`!`, parentheses, and nested field access. You rarely need custom
gateway code.

### 5. "Who's using this? Is it proven?"

Durga is young. It compiles 23 BPMN test fixtures covering the full
BPMN execution subset (tasks, gateways, timers, boundary events,
subprocesses, call activities, message/signal events). The generated
projects compile and run on Quarkus + Kafka.
...
| **Monitoring** | Airflow UI + external tools | Built-in: `process-events` Kafka Streams topology |
| **Error handling** | `on_failure_callback`, retry parameters | Structural: boundary events, DLQ per step, scope cancellation |
| **Audit** | Metadata database (mutable) | Immutable `process-events` topic |
