                ...
                <div class="trend">
                    {#each trendBuckets() as row}
                        <div class="trend-column" title={dateTime(row.bucket)}>
                            <div class="stack">
                                <span class="segment failed"
                                      style={`height: ${percent(row.failed, trendMax(), 0)}`}></span>
                                <span class="segment completed"
                                      style={`height: ${percent(row.completed, trendMax(), 0)}`}></span>
                                <span class="segment started"
                                      style={`height: ${percent(row.started, trendMax(), 0)}`}></span>
                            </div>
                ...
                <section class="panel instance-panel">
                    ...
                    {#if s.instanceView}
                        <dl class="details">
                            <div>
                                <dt>Process</dt>
                                <dd>{s.instanceView.processId ?? '-'}</dd>
                            </div>
                            <div>
                                <dt>Activity</dt>
                                <dd>{s.instanceView.currentActivityId ?? '-'}</dd>
                            </div>
                            <div>
                                <dt>Started</dt>
                                <dd>{dateTime(s.instanceView.startedAt)}</dd>
                            </div>
                            <div>
                                <dt>Updated</dt>
                                <dd>{dateTime(s.instanceView.lastUpdatedAt)}</dd>
                            </div>
                            <div>
                                <dt>Completed</dt>
                                <dd>{dateTime(s.instanceView.completedAt)}</dd>
                            </div>
                            <div>
                                <dt>Retries</dt>
                                <dd>{number(s.instanceView.retryCount)}</dd>
                            </div>
                            <div>
                                <dt>Business key</dt>
                                <dd>{s.instanceView.businessKey ?? '-'}</dd>
                            </div>
                            <div>
                                <dt>Correlation</dt>
                                <dd>{s.instanceView.correlationId ?? '-'}</dd>
                            </div>
                        </dl>
...
<style>
    ...
    * {
    ...
    .segment.started, .legend .started {
        background: var(--blue);
    }

    .segment.completed, .legend .completed {
        background: var(--green);
    }

    .segment.failed, .legend .failed {
        background: var(--red);
    }
